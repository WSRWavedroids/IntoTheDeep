...
public class SparkFunOTOSMod extends I2cDeviceSynchDevice<I2cDeviceSynch> {
    ...
    protected static final byte REG_SCALAR_LATERAL = 0x44;
    ...
    protected SparkFunOTOSMod(I2cDeviceSynch i2cDeviceSynch, boolean deviceClientIsOwned) {
    ...
    /**
     * Gets the lateral scalar used by the OTOS (THIS IS NEW STUFF I ADDED IN)
     * @return lateral scalar
     */
    public double getLateralScalar() {
        // Read the linear scalar from the device
        byte rawScalar = deviceClient.read8(REG_SCALAR_LATERAL);
    ...
    /**
     * Sets the lateral scalar used by the OTOS. Can be used to
     * compensate for scaling issues with the sensor measurements
     * @param scalar Lateral scalar, must be between 0.872 and 1.127
     * @return True if the scalar was set successfully, false otherwise
     * (I ADDED THIS NEW STUFF)
     */
    public boolean setLateralScalar(double scalar) {
        // Check if the scalar is out of bounds
        if (scalar < MIN_SCALAR || scalar > MAX_SCALAR)
            return false;
        ...
        // Write the scalar to the device
        deviceClient.write8(REG_SCALAR_LATERAL, rawScalar);
